...
- ✅ 统一的固定定位和布局
- ✅ 统一的内边距和阴影
- ✅ 合理的 z-index（**100**）- 避免被普通页面元素覆盖；同时在全局样式中统一提升 `wd-overlay` / `wd-popup` 到 **101 / 102**，保证弹层永远高于 `PageFooter`
...
### 错误 55: `day_of_week` 混用数字和英文，显示与排序规则不一致 ⚠️⚠️⚠️

**错误现象**：
```typescript
// UI 批量排课 / 单个排课提交的都是数字字符串
day_of_week: '1' // 周一
day_of_week: '7' // 周日
...
**根本原因**：
- 当前排课 UI（单个排课、批量排课）提交和后端批量排课落库的规范格式是数字字符串 `1`~`7`
- 部分脚本/测试使用英文星期名，部分显示逻辑又只支持英文或只支持数字，导致页面显示/排序不一致

**正确写法**：
```typescript
// ✅ 正确：统一写数字字符串（1=周一 ... 7=周日）
...
**规范**：
- `day_of_week` 的**规范格式**统一为数字字符串：`'1'` / `'2'` / `'3'` / `'4'` / `'5'` / `'6'` / `'7'`
- 映射关系固定：`1=周一 ... 6=周六, 7=周日`
...
### 错误 66: 各端个人中心各写一套退出按钮，导致样式和交互不一致 ⚠️⚠️

**错误现象**：
```vue
<!-- ❌ 错误：管理员/机构/教师中心分别手写不同的退出按钮结构和样式 -->
...
**问题后果**：
- 不同端的个人中心退出按钮圆角、间距、颜色、图标不一致
- 后续要统一视觉风格时需要改多处页面
- 很容易出现有的页面带图标、有的页面无点击态的细节分裂
...
**规范**：
- 管理员端、机构端、教师端等个人中心页的退出入口，必须统一使用公共组件
- 退出按钮样式、图标、点击态放到 `src/components/LogoutButton/` 维护，页面只保留 `handleLogout` 业务逻辑
- 个人中心类页面若出现相同底部操作块，优先抽组件，不要各页面重复写一份样式
